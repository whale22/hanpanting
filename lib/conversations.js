...
import { hasBlockBetweenUsers } from "./user-blocks.js";
...
const ANIMALS = [
  ...
  {
    name: "여우",
    avatarCode: "orange-fox"
  },
  {
    name: "부엉이",
    avatarCode: "purple-owl"
  },
  {
    name: "판다",
    avatarCode: "red-panda"
  },
  {
    name: "돌고래",
    avatarCode: "yellow-dolphin"
  }
];
...
function createAnonymousIdentity() {
  const adjective =
    ANONYMOUS_ADJECTIVES[
      randomInt(ANONYMOUS_ADJECTIVES.length)
    ];
  ...
  return {
    anonymousName: `${adjective} ${animal.name}`,
    avatarCode: animal.avatarCode
  };
}

function createTwoDifferentAnonymousIdentities() {
  const first = createAnonymousIdentity();

  let second = createAnonymousIdentity();

  while (
    second.anonymousName === first.anonymousName
  ) {
    second = createAnonymousIdentity();
  }
...
export async function createConversationForMatchedUsers({
  ...
  if (await hasBlockBetweenUsers(firstUserId, secondUserId)) {
    throw new Error("차단 관계가 있는 사용자는 서로 매칭할 수 없습니다.");
  }
  ...
  const [firstAnonymousIdentity, secondAnonymousIdentity] = createTwoDifferentAnonymousIdentities();
  ...
  const conversation = {
    ...
    participants: [
      createParticipant(
        firstUser,
        firstStance,
        firstAnonymousIdentity.anonymousName,
        firstAnonymousIdentity.avatarCode,
        now
      ),
      createParticipant(
        secondUser,
        secondStance,
        secondAnonymousIdentity.anonymousName,
        secondAnonymousIdentity.avatarCode,
        now
      )
    ],
    status: "ACTIVE",
...
export async function findActiveConversationForUser(userId) {
  if (
...
export async function createMessageForConversation({
  ...
  if (await hasBlockBetweenUsers(userId, otherParticipant.userId)) {
    return {
      ok: false,
      message: "차단 관계가 있는 사용자와는 메시지를 주고받을 수 없습니다."
    };
  }
...
export async function findConversationCountsForUser(
  userId
) {
  const emptyCounts = {
    sameMatchCount: 0,
